...
class PapermillEngines(object):
    ...
    async def execute_notebook_with_engine(self, engine_name, nb, kernel_name, **kwargs):
        """Fetch a named engine and execute the nb object against it."""
        return await self.get_engine(engine_name).execute_notebook(nb, kernel_name, **kwargs)
...
class Engine(object):
    ...
    async def execute_notebook(
        ...
        **kwargs,
        ...
        try:
            await cls.execute_managed_notebook(nb_man, kernel_name, log_output=log_output, **kwargs)
        finally:
            nb_man.cleanup_pbar()
    ...
    @classmethod
    async def execute_managed_notebook(cls, nb_man, kernel_name, **kwargs):
...
class NBClientEngine(Engine):
    ...
    @classmethod
    async def execute_managed_notebook(
        cls,
        nb_man,
        kernel_name,
        log_output=False,
        ...
        **kwargs,
        ...
        return await PapermillNotebookClient(nb_man, **final_kwargs).execute()
